...
class TagFilterUI():
    ...
    def set_callbacks(self):
        ...
        self.cb_prefix.change(fn=lambda x: self.cb_prefix_changed(x), inputs=self.cb_prefix, outputs=self.cbg_tags)
        self.cb_suffix.change(fn=self.cb_suffix_changed, inputs=self.cb_suffix, outputs=self.cbg_tags)
        self.cb_regex.change(fn=lambda x: self.cb_regex_changed(x), inputs=self.cb_regex, outputs=self.cbg_tags)
        self.rb_sort_by.change(fn=self.rd_sort_by_changed, inputs=self.rb_sort_by, outputs=self.cbg_tags)
        self.rb_sort_order.change(fn=lambda x: self.rd_sort_order_changed(x), inputs=self.rb_sort_order,outputs=self.cbg_tags)
