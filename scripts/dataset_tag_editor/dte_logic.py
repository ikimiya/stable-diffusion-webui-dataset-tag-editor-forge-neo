...
class DatasetTagEditor(Singleton):
    ...
    def __init__(self):
        ...
        _word_regex = getattr(shared.opts, 'dataset_filename_word_regex', '')
        self.re_word = re.compile(_word_regex) if len(_word_regex) > 0 else None
    ...
    def load_interrogators(self):
        ...
        def read_wd_batchsize(name: str):
            if "vit" in name:
                return getattr(shared.opts, 'dataset_editor_batch_size_vit', 4)
            elif "convnext" in name:
                return getattr(shared.opts, 'dataset_editor_batch_size_convnext', 4)
            elif "swinv2" in name:
                return getattr(shared.opts, 'dataset_editor_batch_size_swinv2', 4)
            else:
                return 4
        ...
        self.INTERROGATORS = (
                ...
                + ([taggers_builtin.DeepDanbooru()] if getattr(taggers_builtin, 'DeepDanbooru', None) else [])
                + [
        ...
        try:
            ...
            for it in self.INTERROGATORS:
                if it.name() == interrogator_name:
                    if getattr(taggers_builtin, 'DeepDanbooru', None) and isinstance(it, taggers_builtin.DeepDanbooru):
                        with it as tg:
                            res = tg.predict(img, threshold_booru)
                    elif isinstance(it, taggers_builtin.WaifuDiffusion) or isinstance(it,
